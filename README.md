...
This removes the static-grid, forcing, isolated-kernel, event-binding, execution, prediction-freeze and reference-identity blockers without hiding the ten effective terrain gaps or the NoData pixels inside partially covered windows. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction, but they are not on the public-only evaluation critical path. The next gate is to normalize and rasterize each acquired reference independently on the frozen grid before any metric is run. Any negative result must be retained, and any later EA model delivery remains an optional comparison or separately versioned physics upgrade.
