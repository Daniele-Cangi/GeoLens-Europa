...
- a frozen event-runner contract that fixes clean-revision authorization, lazy one-interval forcing construction, streaming maximum-depth aggregation, exact 900-second outputs, adaptive-CFL bounds, per-step and final mass balance, the all-nine-scenarios completion rule, NoData encodings and physical wetness thresholds. Two interrupted operational attempts produced only partial content-addressed artifacts and never created a prediction receipt; observed flood geometry remains sealed. Runner v0.2 adds an authorization-bound, atomically written checkpoint after every complete scenario so a machine restart resumes from the verified contiguous prefix instead of repeating completed work.
...
Event-runner contract SHA-256 `b9f2b9d9f91557b4cd62f35875fa2b16d3ebf8fe5e536416cf2d9bcb0dae23b9` fixes the execution and prediction semantics independently of process lifetime. `npm run preflight:cumbria-event-run -- --data-root <external-directory>` checks 62 artifact descriptors / 59 unique artifacts, all source and transformation hashes, the 288-interval schedule and the nine-scenario order without writing authorization, checkpoint or prediction files. `--freeze` is valid only from an allowed branch with a clean Git tree and writes the versioned content-addressed authorization. During `--execute`, each completed scenario first passes output, stability, mass-balance and artifact verification, then receives an atomic checkpoint bound to that authorization, Git tree, manifest, input receipts and exact scenario definition. A restart may reuse only one contiguous verified checkpoint prefix; a gap, changed byte, changed binding or changed identity fails closed. Checkpoints remain partial execution evidence, never a prediction. The final prediction receipt is created only after all nine scenarios pass, and both observed flood references remain sealed until then.
...
This removes the static-grid, forcing, isolated-kernel, event-binding and runner-contract blockers without hiding the ten effective terrain gaps or the NoData pixels inside partially covered windows. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction, but they are not on the critical path. The next engineering gate is deliberately operational: after this checkpoint is merged, bind the unchanged sources, manifest and external receipts to the clean Git commit and tree, then execute all nine scenarios. A complete, physically admissible, content-addressed prediction—not a partial run—may unlock blind evaluation. Any later EA delivery will be reviewed as an optional comparison or versioned physics upgrade.
