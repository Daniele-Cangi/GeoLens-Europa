...
| Cumbria 2015 | A small public-only River Eden experiment can advance without waiting for the official archived model | The 56 km² Sheepmount–Old Sandsfield domain now has content-addressed real DTM, CLC 2012 and IMERG evidence; Sheepmount supplies a complete input-candidate flow series, and the blind evaluation references remain sealed | Ten effective terrain grids remain missing, while channel placement, downstream/initial conditions, roughness, computation grid, timestep and the prediction remain unresolved |
...
The Carlisle candidate is no longer just a list of promising portals. GeoLens has frozen a public-only downstream-reach domain, a fail-closed hydraulic input protocol and a sealed blind-evaluation boundary for Storm Desmond from `2015-12-04T00:00:00Z` to `2015-12-07T00:00:00Z`. The Environment Agency model request is useful, but it is no longer on the critical path.
...
This removes the bounded environmental-input blocker without hiding the ten effective terrain gaps or the NoData pixels inside partially covered windows. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction, but they are not on the critical path. GeoLens can now compose the three real source grids for inspection and freeze an explicitly experimental replacement-solver contract. Any later EA delivery will be reviewed as an optional comparison or physics upgrade. The two observed flood extents remain unavailable to model input and calibration until the prediction is frozen.
...
The CLC and IMERG commands are dry runs unless `--execute` is supplied. Both reject a data root inside the repository or OneDrive. IMERG reuses the canonical NASA cache, so a verified repeat does not download the 144 granules again.
