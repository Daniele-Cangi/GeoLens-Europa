...
| Cumbria 2015 | Public evidence now defines the historical Carlisle model domain and exact model records to request without pretending the replay is runnable | A fixed Storm Desmond window, four typed upstream discharge series, the documented Old Sandsfield downstream limit, complete direct observations, complete IMERG catalogue coverage, pre-event model lineage, current-asset qualification, evaluation isolation and a deterministic pre-event terrain-catalogue selection | Model cross-sections/files, boundary placement and values, initial conditions, ten terrain grid references, downloadable raster mapping and an as-of-event defence/channel state remain unresolved |
