...
The blind evaluation boundary is now frozen around the completed prediction under protocol SHA-256 `1a135785bef1121e542952fd8ee90d6eed86908d19864d381985fbfd2f8a1dd0`. It pins the primary `primary-20m` maximum-depth mask at `>= 0.05 m`, its compressed and decoded identities, and the 73,502-cell valid-prediction mask on the 400 × 350 EPSG:27700 grid. Six metrics remain fixed—intersection over union, area precision, area recall, false-positive area, false-negative area and symmetric 95th-percentile boundary distance. Missing observed coverage is excluded and reported rather than treated as dry; missing prediction coverage blocks evaluation. Environment Agency and Copernicus extents must be acquired and evaluated separately, so disagreement remains visible. Their geometry cannot select the domain, mesh, threshold or model parameters. `npm run verify:cumbria-blind-evaluation-protocol -- --data-root <external-directory>` verifies the source commit and manifest, final receipt, all nine checkpoints and all 72 compressed/decoded prediction artifacts without opening either reference.
...
This removes the static-grid, forcing, isolated-kernel, event-binding, execution and prediction-freeze blockers without hiding the ten effective terrain gaps or the NoData pixels inside partially covered windows. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction, but they are not on the public-only evaluation critical path. After this checkpoint is merged, the next gate is to acquire and content-address only the declared Environment Agency and Copernicus evaluation references, then compare each independently with the frozen prediction. Any negative result must be retained, and any later EA model delivery remains an optional comparison or separately versioned physics upgrade.
