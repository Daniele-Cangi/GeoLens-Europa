...
| Cumbria 2015 | A small public-only River Eden experiment can advance without waiting for the official archived model | The 56 km² domain, real DTM/CLC/IMERG evidence, three computation grids, complete event forcing and the local-inertial kernel are verified independently before evaluation | Terrain gaps remain explicit; event-input binding is not yet verified, so no Storm Desmond run or prediction exists |
...
Work can continue on the Cumbria gates that were already declared: fail-closed event-input binding, event execution, prediction freeze and blind evaluation. Package integrity checks, minimal format/CRS/unit/schema adapters, tests, security, reproducibility and interface work can also continue.
