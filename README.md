...
The Python/NumPy kernel `cumbria-local-inertial-surface-flow-v0.1.0` now passes a content-addressed, event-isolated fixture suite. The tests cover lake-at-rest equilibrium over variable terrain, exact closed-cell rainfall storage, non-negative sloped free outflow, exact forcing/output time boundaries and mandatory failure when CFL stability would require a timestep below 0.05 s. Missing forcing is rejected. The fixture result SHA-256 is `cd31a9e9cbfb41e44d9619e4a1d9bdd1fc34fa81ce877bbf596772e5c80704c1`; the suite reads no Storm Desmond inputs or observed flood geometry, performs no network request or external write and does not create a prediction.

The event-input binding receipt is SHA-256 `d0ebae08ebef1a1ca08b83977b9110eedbcf70d9290964c05ae20f8ca022500a`. Its 19 unique artifacts occupy 1,097,674 compressed bytes and represent 6,555,387 decoded bytes. CSR mappings retain the 12 native IMERG cell identities and cover every valid solver cell within the frozen tolerance; the 60 m, 100 m and 140 m Sheepmount footprint variants retain their exact valid-area weights. `npm run materialize:cumbria-event-input-binding -- --data-root <external-directory> --check` verifies the grids, masks, parameter ranges, forcing arrays, units, timing, mappings and receipt with zero writes. It still authorizes zero event runs and zero evaluation access.
...
This removes the static-grid, forcing and isolated-kernel blockers without hiding the ten effective terrain gaps or the NoData pixels inside partially covered windows. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction, but they are not on the critical path. The next engineering gate is a fail-closed, content-addressed binding from the materialized grids and event forcing into the verified kernel. It must be verified before Storm Desmond is executed. Any later EA delivery will be reviewed as an optional comparison or physics upgrade. The two observed flood extents remain sealed until a prediction is frozen.
