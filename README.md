...
![Cumbria blind-evaluation spatial error map](docs/screenshots/cumbria-blind-evaluation.png)
...
This removes the static-grid, forcing, isolated-kernel, event-binding, execution, prediction-freeze, reference-identity and blind-comparison blockers without hiding the ten effective terrain gaps or the NoData pixels inside partially covered windows. The result demonstrates that the current public-only physics is not spatially adequate as a Carlisle flood reconstruction. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction. The next gate is failure analysis that preserves this frozen result; any later EA model delivery remains an optional comparison or separately versioned physics upgrade.
