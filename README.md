...
This removes the first composition blocker without hiding the ten effective terrain gaps or the NoData pixels inside partially covered windows. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction, but they are not on the critical path. The next engineering gate is an explicitly experimental public-only replacement-solver contract with declared geometry, boundary and initial-state assumptions; the H3 inspection result cannot supply those choices. Any later EA delivery will be reviewed as an optional comparison or physics upgrade. The two observed flood extents remain unavailable to model input and calibration until the prediction is frozen.
...
The CLC, IMERG and spatial-evidence commands are dry runs unless `--execute` is supplied. All reject a data root inside the repository or OneDrive. IMERG reuses the canonical NASA cache, so a verified repeat does not download the 144 granules again. For the composed cell, `--check` rebuilds the result in memory and compares the receipt and compressed artifact byte for byte without writing files.
