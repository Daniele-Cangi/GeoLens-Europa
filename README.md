...
| Cumbria 2015 | Public evidence now defines the historical Carlisle model domain, exact model records to request and the bounded pre-event terrain delivery without pretending the replay is runnable | A fixed Storm Desmond window, four typed upstream discharge series, the documented Old Sandsfield downstream limit, complete direct observations, complete IMERG catalogue coverage, pre-event model lineage, current-asset qualification, evaluation isolation and 30 traceable DTM archive identities | Model cross-sections/files, boundary placement and values, initial conditions, ten explicitly missing terrain grid references and an as-of-event defence/channel state remain unresolved |
