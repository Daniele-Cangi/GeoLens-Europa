...
The bounded inputs live outside Git under `D:/GeoLens/data/emilia-romagna-2023`. Manifest v1.14.0 pins 55 official and derived artifacts totaling 746,130,293 bytes, including the canonical IMERG cache and portable source grid, DBTR GeoPackage and metadata, ten DBTR masks/coverage arrays, four terrain-routing arrays, five event-runoff arrays, the bounded PST audit, the restricted evaluation mask and receipt, the archived official Commission report, the official event monograph, and the audited PAI relation/DWG archives. No observed geometry, source archive or derived mask is committed to Git. Styled WMS images remain per-run context receipts and are never promoted to physical evidence.
