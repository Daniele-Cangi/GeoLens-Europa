...
- a frozen event-runner contract that fixes the clean-revision authorization, lazy one-interval forcing construction, streaming maximum-depth aggregation, exact 900-second outputs, adaptive-CFL bounds, per-step and final mass balance, all-nine-scenarios completion rule, NoData encodings and physical wetness thresholds before the first Storm Desmond execution. Preflight has passed against the real external artifacts; no solver run or observed flood geometry has been opened.
...
Event-runner contract SHA-256 `802279c17252fbe39f2cf2fc5b6d229369a9a5030b98fdfa6c0993222bb76cee` fixes the execution and prediction semantics before any event output exists. `npm run preflight:cumbria-event-run -- --data-root <external-directory>` checks 62 artifact descriptors / 59 unique artifacts, all source and transformation hashes, the 288-interval schedule and the nine-scenario order without writing authorization or prediction files. `--freeze` is intentionally valid only from an allowed branch with a clean Git tree and writes a content-addressed authorization; `--execute` requires that exact authorization and creates a prediction receipt only if every scenario passes the frozen stability and mass-balance checks. Until the authorization is created after merge, solver runs remain exactly zero and both observed flood references remain sealed.
