...
| Cumbria 2015 | A small public-only River Eden experiment can now advance without waiting for the official archived model | The 56 km² Sheepmount–Old Sandsfield domain is frozen without observed flood geometry; 52 of 56 source grids map to six pre-event 1 m DTM archives, Sheepmount supplies a complete input-candidate flow series, and the blind evaluation references remain sealed | Four terrain grids remain missing; terrain is not yet materialized, and channel placement, downstream/initial conditions, roughness, computation grid, timestep and the prediction remain unresolved |
...
- 52 covered 1 km terrain references mapped to six official pre-event 1 m DTM archives; `NY3256`, `NY3257`, `NY3357` and `NY3959` remain explicit NoData;
- an estimated 600 MB decoded full-archive footprint and 208 MB retained terrain payload, before ZIP and raster-format overhead, with one-at-a-time acquisition authorized but solver execution still blocked.
...
Manifest v0.15.0 retains the conservative 30-archive full-envelope plan but adds the smaller public baseline that we will actually acquire first. Its domain is the 8 km × 7 km British National Grid rectangle containing Sheepmount and Old Sandsfield with approximately 1 km terminal and lateral margin. It selects six official 1 m DTM archives and masks them back to 52 eligible 1 km grids. The four uncovered grids remain NoData rather than zero or interpolated terrain. The estimate is 600 MB for all six decoded archives and 208 MB for the retained source pixels; ZIP size and temporary format overhead are reported separately when downloaded. This authorization applies only to those six archives and does not authorize the solver or evaluation-reference access.
...
This removes the public-baseline domain and terrain-selection blockers without hiding its four real coverage gaps. The archived EA model, channel sections, historical boundary values and defence state remain unavailable for an official-model reconstruction, but they are not on the critical path. GeoLens can now materialize the six selected terrain archives, then freeze an explicitly experimental replacement-solver contract. Any later EA delivery will be reviewed as an optional comparison or physics upgrade. The two observed flood extents remain unavailable to model input and calibration until the prediction is frozen.
