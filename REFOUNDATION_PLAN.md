...
State: public Sheepmount–Old Sandsfield domain, bounded real DTM/CLC/IMERG evidence, corrected 10 m, 20 m and 40 m static solver grids, complete native 15-minute/30-minute forcing, the fixture-verified local-inertial kernel, fail-closed event-input binding and event-runner contract are reproduced; clean-revision authorization, Storm Desmond execution, prediction freeze and blind evaluation remain gated

Progress:

- manifest v0.24.0 freezes a bounded Carlisle source-discovery envelope, a separate local hydraulic-protocol envelope, the public Sheepmount–Old Sandsfield baseline domain, verified DTM/CLC/IMERG materialization and single-cell composition results, the replacement-solver contract, corrected static solver grids, complete time-varying forcing, isolated numerical-kernel verification, event-input binding, event-runner contract, and the half-open 72-hour Storm Desmond window from 4 December 2015 00:00 UTC to 7 December 2015 00:00 UTC;
...
- event-runner contract SHA-256 `802279c17252fbe39f2cf2fc5b6d229369a9a5030b98fdfa6c0993222bb76cee` freezes lazy one-interval forcing, streamed maximum-depth and time-of-maximum aggregation, exact 900-second outputs, adaptive-CFL bounds, per-step/final mass balance, NoData encodings, physical wetness thresholds and the all-nine-scenarios completion rule before execution;
...
- after merge, create the content-addressed execution authorization from the clean Git commit and tree without changing the frozen model or output semantics;
...
- the event runner reproduces its pinned source identities and contract, loads at most one forcing interval at a time, streams rather than retains all depth snapshots, and cannot execute without a clean-revision authorization matching every frozen dependency;
