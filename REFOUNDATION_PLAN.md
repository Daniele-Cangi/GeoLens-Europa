...
- declared IMERG, CLC, GLO-30, XDBTR and event-2 coverage all contain the bounded pilot; manifest v1.2.0 originally froze the common EPSG:4326 bounds, which current manifest v1.14.0 retains together with a globally aligned EPSG:32632 30 m grid of 335 by 420 cells, while H3 r11 remains a separate representation choice;
...
- manifest v1.14.0 and the external-data verifier now pin 55 benchmark artifacts totaling 746,130,293 bytes, including the WCS description, bounded PST audit, archived official Commission report, official breach-repair HTML, special-plan monograph, PAI relation and three inspected DWG archives.
...
- acquire the source numerical Montone/Castrocaro discharge series or ARPAE 2022 rating-curve definition together with a full-window Rabbi discharge relation/hydrograph, a defensible downstream boundary, source-backed breach timing/geometry, and a complementary terrain/channel source before the frozen conditioned replay can become eligible; the plotted Commission figure and public PAI drawing archives do not satisfy this requirement;
