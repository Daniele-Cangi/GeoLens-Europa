...
State: the public Sheepmount–Old Sandsfield Storm Desmond run and the single predeclared blind evaluation are complete; external receipt SHA-256 `610bae9d7e31978e3565a5e04e779bde9e1dc5236e200a0699cf8db9c118ffa2` retains a negative baseline with low spatial agreement and large false-positive area, while diagnostic receipt SHA-256 `edccd01df0467fb6e3a9d0302ae4869f1d46269aa48901a866f854ca7d0dc721` makes that failure spatially inspectable without retuning, metric recomputation or another evaluation

Progress:

- manifest v0.31.0 retains the v0.25.0 execution source identity, the v0.26.0 prediction freeze, the raw and normalized reference receipts, the v0.29.0 authorization, the one completed evaluation and its presentation-only diagnostics without rewriting any historical boundary; the run used merged commit `0f4518b`, the reviewed evaluator bytes from `5d34bc1`, the primary 20 m maximum-depth mask at `>= 0.05 m`, the same 73,502-cell evaluation-domain mask and the half-open 72-hour Storm Desmond window;
...
- analyze the retained false-positive pattern and missing physics without changing or replacing the frozen baseline; any revised experiment must preserve the original result beside it.
