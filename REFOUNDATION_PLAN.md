...
- reproduce and audit the materialized 15-minute Sheepmount and 30-minute IMERG forcing under the already frozen transformations;
- bind the already materialized Cumbria grids and forcing to the verified kernel through a fail-closed, content-addressed event-input contract;
...
State: the public Sheepmount–Old Sandsfield Storm Desmond run and the single predeclared blind evaluation are complete; external receipt SHA-256 `610bae9d7e31978e3565a5e04e779bde9e1dc5236e200a0699cf8db9c118ffa2` retains a negative baseline with low spatial agreement and large false-positive area, diagnostic and inventory receipts make that failure inspectable, and the v0.33 physics-revision gate now blocks retrospective solver changes or Carlisle reruns until event-valid evidence and an independently merged fixture protocol exist

Progress:

- manifest v0.33.0 retains the v0.25.0 execution source identity, the v0.26.0 prediction freeze, the raw and normalized reference receipts, the v0.29.0 authorization, the one completed evaluation, its presentation-only diagnostics, descriptive failure inventory and post-evaluation revision gate without rewriting any historical boundary; the run used merged commit `0f4518b`, the reviewed evaluator bytes from `5d34bc1`, the primary 20 m maximum-depth mask at `>= 0.05 m`, the same 73,502-cell evaluation-domain mask and the half-open 72-hour Storm Desmond window;
...
- CLC 2012 uses 2011-2012 reference imagery that predates the event, while its V2020_20u1 corrected release remains disclosed as retrospective processing; the official EEA raster service materialized 7,553 available native 100 m EPSG:3035 cells across 12 observed classes with zero missing cells;
...
- the public domain contains 56 one-kilometre source grids: 52 map deterministically at catalogue level to six official pre-event 1 m DTM archives and four (`NY3256`, `NY3257`, `NY3357`, `NY3959`) are absent before download; catalogue selection SHA-256 is `1b018876cccd284c9e0bc514d5dbb73aa69712a4c4d18b8c366221fefcf88667`, archive selection SHA-256 is `209f45bd8e823ab1c49a411920fe513bbe269e547c479570bade878772999130` and the unchanged blind protocol hash is `7fe1d804a5bbe8e5603d484422a9dce106960c90b26e38a83fb8d1cae30e3689`;
- the six official ZIP archives are now materialized outside Git: 280,161,858 archive bytes, 40 unique source GeoTIFFs and source receipt SHA-256 `4ea7faa7f54f4a3e525d26e4324e6a56a86e2813ea6ae6cef07bf0d342a0cd8d`;
- georeferenced archive inspection maps 48 exact 1 km windows; six use explicitly recorded older pre-event rasters already present in the selected archive set, including `NY3758`, whose selected 2013 window contained only NoData but whose 2009 window contains real samples;
- 46 windows contain real elevation evidence, while `NY3859` and `NY3960` contain only provider-declared NoData; four further catalogue-selected cells (`NY3258`, `NY3259`, `NY3358`, `NY3359`) have no covering raster in the selected or already materialized older archive for the same tile, producing ten effective missing grids when combined with the catalogue gaps;
- the 48 mapped windows contain 30,678,994 valid Float32 elevations and 17,321,006 NoData pixels; deterministic gzip storage occupies 36,148,351 physical bytes for 192,000,000 decoded bytes, with mask receipt SHA-256 `c9acfe46f41e08e40e6473ce399e912b8d4e27c880e928e0f1a77aef15749988`; neither missing pixels nor missing grids are converted to zero;
- the bounded CLC materialization receipt is pinned as SHA-256 `dce61b2234329619ce1212ccc3a49650c1fec68eea7bc5d465f722e170ebc96d`; the decoded categorical grid is pinned as `35214149c6bc51e703c766f6e06b5ddb28955e5aded709105acdf1aa3536d779`, categorical interpolation is forbidden and unknown rendered colours fail;
- the bounded IMERG materialization receipt is pinned as SHA-256 `fb768f0de5dd2e39df8c32e80655b28e9dfef02d1ed82605eb94012bb244ebf7`; its portable native-grid artifact is pinned as `99bb25ba7ed8ae874eb44b711b47457948e29495f41c3c06c0f29b5f0158be94`, source granules remain in the canonical NASA cache and no H3 precision claim is introduced;
...
- the conservative full-envelope archives still require SHA-256 content-addressed receipts and bounded ZIP inspection that rejects encryption, links/reparse points, absolute paths, parent traversal and duplicate normalized paths; `npm run plan:cumbria-dtm-materialization` continues to emit that separate 30-archive dry-run with zero network requests and zero file writes, while the smaller public-baseline path is implemented by `materialize:cumbria-dtm` and `materialize:cumbria-dtm-masks`;
...
- EPSG:27700 remains the exchange coordinate frame for source-overlap work. A separate replacement contract now fixes the public-only 8 km × 7 km computation extent, upper-left origin and a 20 m primary mesh with 10 m and 40 m predeclared resolution sensitivities; these computation grids do not redefine DTM, CLC, IMERG or H3 source precision, and `npm run plan:cumbria-spatial-grid` continues to recompute the H3 index identity in a zero-network, zero-write dry run;
...
- replacement protocol `cumbria-public-surface-flow-replacement-v0` is frozen independently as canonical SHA-256 `b9db1fbc10cc0aeff5d3a4e24bf1afc5d66226902ecad92ce111f1fcfb60c89b`; it defines an explicitly experimental 2D local-inertial surface-flow formulation rather than claiming to reconstruct the unavailable ISIS/TUFLOW channel model;
- the primary computation grid is 400 × 350 at 20 m over `[332000, 556000, 340000, 563000]` in EPSG:27700, with mandatory 10 m and 40 m resolution sensitivities; native DTM coverage must be complete inside a computation cell or that cell remains missing, its faces are no-flux and the adjacent prediction halo is excluded rather than called dry;
- IMERG is transformed by native-footprint overlap into piecewise-constant 30-minute intensity, CLC parameters remain explicit uncalibrated assumptions with fixed low/primary/high runoff and Manning values for the 12 observed classes, and unknown or missing input blocks execution instead of becoming zero;
- Sheepmount contributes a versioned positive-discharge-excess proxy above the first event-window sample, held over each native 15-minute interval and distributed over an explicit 100 m footprint with 60 m and 140 m sensitivities; it is not asserted to be total channel flow or a historical cross-section;
- the replacement baseline starts with zero surface depth and discharge as a named dry-surface assumption, uses zero-gradient free outflow without an imposed Old Sandsfield stage, omits unavailable event-valid channels/defences/floodgates, integrates with adaptive CFL `0.7` over `0.05–5 s`, and must fail rather than step below the declared minimum;
- nine scenarios are fixed before evaluation and all must be reported: primary 20 m, 10 m/40 m mesh, low/high runoff, low/high Manning roughness and 60 m/140 m inflow-footprint variants. The primary wetness semantic is maximum event depth at or above 0.05 m, with 0.01/0.10/0.30 m reporting thresholds; best-scenario selection and evaluation-driven retuning are forbidden;
- `npm run verify:cumbria-replacement-solver-protocol` reproduces the immutable protocol identity and mesh accounting with zero network requests, zero writes, zero solver runs and zero evaluation-reference loads;
- `npm run verify:cumbria-local-inertial-kernel` reproduces v0.2 fixture result SHA-256 `b7ac171c9b28ab6bf69ff6cb8d3c43d07c102444483d8e883de82739fcd6a423` for lake-at-rest equilibrium, conservative rainfall, non-negative free outflow, exact source/output alignment, lazy forcing, streamed output observation and below-minimum timestep failure. It reads zero event inputs and zero evaluation geometries, makes zero network requests or external writes and does not authorize event execution;
- blind evaluation protocol `carlisle-blind-inundation-evaluation-v0` is frozen before either post-event geometry is loaded: prediction receipt, primary wet-mask bytes/content, code revision, model/transformation versions, physically defined 0.05 m wetness semantic and EPSG:27700 evaluation-domain mask are now pinned;
...
- the protocol forbids reference-driven domain, mesh, threshold or calibration choices and visual inspection before prediction freeze; `npm run verify:cumbria-blind-evaluation-protocol -- --data-root <external-directory>` pins SHA-256 `1a135785bef1121e542952fd8ee90d6eed86908d19864d381985fbfd2f8a1dd0` and verifies the historical source manifest, final receipt, nine checkpoints and 72 compressed/decoded output artifacts with zero network requests, zero writes and zero evaluation runs;
...
- Environment Agency response `EIR2026/42104` was received on 18 September 2026 and stored outside Git on the D volume as one 46,688,104,848-byte ZIP with independently reproduced SHA-256 `40ae1d7df10b105b8abb195c43b53e9576683a654e781686406f286cee5da145`;
- the package contains four top-level historical archives: Carlisle surveys, the 2007 Eden ABD extract, the 2010 Carlisle FAS package and the 2012 Cumbria Tidal package. Central-directory inspection found 2,787 files including TUFLOW/ISIS material, reports, outputs, cross-sections, terrain and survey data, with zero encrypted files, unsafe paths or duplicate normalized paths; nested content remains unextracted;
- the registered package remains `received`, not reviewed: all ten components are incomplete or metadata-only pending model-group mapping, temporal lineage, embedded third-party licence coverage, CRS/units/datum and completeness review. A separate review receipt must decide every component and verify the sealed evaluation boundary. Accepted required components become candidates for a separate physical-gate assessment; intake can never grant replay eligibility itself, while the older frozen access-protocol manifest retains its historical `packageReceived: false` state;
...
- after prediction-freeze merge `df33838`, fixed domain plus provider event group `4175` select only Environment Agency feature `Recorded_Flood_Outlines.26355`; its 53,114 raw GeoJSON bytes are pinned as SHA-256 `e2ad395a39441a077cf3585e3cce09a49454922798dba483a31fe71fabef78c0`;
- both official Copernicus EMSR147 Carlisle vector archives are content-addressed separately: initial SHA-256 `5524efba986082b901bf27fc7ecde0cf6af91fea393b36bfa6bcf6a959448049` and monitoring SHA-256 `b58b6e047d8e1066fffcf3aa09fafa4b975cd7eedccda22d8df8c4971d509cfb`; ZIP inventory is verified without extraction and their crisis layers retain 228 and 495 records respectively;
- evaluation-reference receipt SHA-256 `b9bf772af4a356de533edb26c005dd318e36d889ad44fdaeb51586c989adffbf` binds all three raw sources to the already-frozen prediction and records zero model-input use, zero calibration, zero prediction selection, zero evaluation runs and no reference combination;
- evaluation-reference normalization receipt SHA-256 `fae2cadba3675bff4191da5829e8bf64d71ffc028a9c6899c9e865f97b6debe0` independently transforms the EA outline and both EMSR147 products with pinned 2 m coordinate operations, clips only to the pre-frozen EPSG:27700 domain and applies one 20 m cell-centre rule; the masks retain `1` mapped flooded, `0` covered but not mapped flooded and `255` missing coverage, while the present three publisher coverages each contain all 140,000 grid cells;
- the three independent wet masks contain 3,314 EA cells, 988 EMSR147 initial cells and 3,336 EMSR147 monitoring cells; these are transformation diagnostics rather than accuracy results, no prediction artifact was loaded, and no evaluation metric has run;
- the execution authorization binds protocol, frozen prediction, normalized references and the corrected evaluator source after review; its isolation record remains at zero loaded artifacts, writes, network requests and evaluation runs, so authorization is not itself an accuracy result;
...
- bounded real Cumbria inputs, completed predictions and raw evaluation references are registered outside Git with separate content identities; opening the references occurred only after prediction freeze, and no observed geometry has entered input, calibration or scenario selection.
- the frozen 10 m, 20 m and 40 m meshes are materialized directly from native DTM and CLC footprints as 36 unique content-addressed artifacts; corrected receipt SHA-256 `63fa37941c13cc6a18bb3fed9f9ab1690ae7da34da9963cead2299adc879800c` represents 25,725,000 decoded bytes in 2,342,503 compressed physical bytes;
- fail-closed binding validation caught that the superseded grid receipt had valid CLC masks but NoData runoff and Manning arrays because its accumulators began at `NaN`; preprocessing v0.2.0 fixes the defect, verifies finite parameters on every land-cover-valid cell and keeps the superseded receipt outside Git but forbidden as solver input;
...
- all 144 native half-hour IMERG amount grids and 288 native 15-minute Sheepmount flow samples are content-addressed under receipt SHA-256 `782eb107578fe34bccba9bc0202d90d06d504ddd63bc03ba301b1ccb3781c38b`; the IMERG series reproduces the previously pinned accumulation with exactly 0 mm maximum difference under the original Float32 arithmetic;
- Sheepmount observed flow remains separate from the versioned positive-excess proxy above the first 422.716 m3/s sample; the derived excess volume is 118,482,930.9 m3, missing intervals would fail rather than be filled, and the forcing materialization performed zero solver and evaluation runs;
- `npm run materialize:cumbria-forcing -- --data-root <external-directory> --check` verifies the receipt and all four compact artifacts offline with zero writes.
- event-input receipt v0.2 SHA-256 `502a1ecad80e0f1877f853967524c13a54d4ce52cddfbefc149873f8591134d4` binds the verified v0.2 kernel to all nine scenarios through the same 19 unique content-addressed mapping artifacts: 399,523 valid cells have complete exact-footprint IMERG coverage, source timing is fixed at 288 left-closed 900-second intervals, and Sheepmount discharge is distributed only by intersected valid footprint area; the superseded receipt remains preserved but cannot authorize a run;
- `npm run materialize:cumbria-event-input-binding -- --data-root <external-directory> --check` re-verifies every input byte/hash, validity mask, finite parameter range, forcing value, unit, source index, temporal rule and persisted mapping with zero network requests, writes, solver runs, prediction artifacts or evaluation access.
- two interrupted execution attempts demonstrated that content-addressed output arrays alone cannot recover the in-memory timestep and mass-balance summaries required by the final receipt. They produced 52 unique partial artifacts for seven scenarios, but no complete scenario checkpoint and no prediction receipt; those artifacts remain traceable external material and cannot be promoted into a prediction;
- event-runner contract SHA-256 `b9f2b9d9f91557b4cd62f35875fa2b16d3ebf8fe5e536416cf2d9bcb0dae23b9` preserves the same numerical and scientific semantics while adding atomic per-scenario checkpoints. Each checkpoint binds authorization, Git tree, manifest, input receipts, exact scenario definition, stability summary, mass balance and all output byte/content hashes; only a contiguous verified prefix may be reused after restart, and any gap or drift fails closed;
- `npm run preflight:cumbria-event-run -- --data-root <external-directory>` verifies 62 artifact descriptors / 59 unique artifacts, source and transformation identities, schedule and scenario order against the real external package with zero writes, solver runs, network calls or evaluation access. Authorization can be created only after merge from an allowed branch with a clean Git tree and records the exact commit, tree, manifest, source and input-receipt hashes;
- authorization SHA-256 `7feda0564923260c2996f292ec4ae82b52a6cd7a17b32aaca805a90816a4ab44` bound the execution to merge commit `1cab9bfccbfecd0818ed7f8e7b8735bd4b437bb1`, tree `91065ade3d0cf8a3b5e972f23554234eb52caf66` and source manifest SHA-256 `f43632afaca8d9a86a4c3a7cdb955519b6ca89f170f27dcd0f772f75bcf19fab`;
- the restart-safe run completed `primary-20m`, the 10 m/40 m mesh sensitivities, low/high runoff, low/high roughness and 60 m/140 m inflow-footprint scenarios. Every checkpoint has 288 outputs and passed the frozen stability, non-negative finite-depth, artifact-integrity and mass-balance gates;
- final prediction receipt SHA-256 `f2a3a7489699a70a6d5c770633bdc9f789184ca26cb8190c85a1d28d40495dc6` records all nine results and 72 artifact descriptors with no best-scenario selection, evaluation-driven retuning, network request or observed-flood access;
- the primary evaluation artifact is the `primary-20m` maximum-event-depth mask at `>= 0.05 m`, compressed SHA-256 `dffe515490e25f371095547f4b6e90f93b5744252bf4a82948b846099a17575f`, evaluated only where the frozen valid-prediction mask SHA-256 `2273498b8cacfaace9245ae4f4f20a57077ad45758826919843e8837af338230` is available;
- the single authorized comparison completed with zero network requests and no threshold, scenario or model change. Against the Environment Agency outline, IoU is `0.0547`, precision `0.0585`, recall `0.4536`, false-positive area `5.407 km2`, false-negative area `0.405 km2` and p95 boundary distance `7.446 km`;
- against EMSR147 initial, IoU is `0.0140`, precision `0.0141`, recall `0.5833`, false-positive area `5.662 km2`, false-negative area `0.058 km2` and p95 boundary distance `2.371 km`; against monitoring-01, IoU is `0.0303`, precision `0.0327`, recall `0.2982`, false-positive area `5.555 km2`, false-negative area `0.442 km2` and p95 boundary distance `2.272 km`;
- the frozen model marks 14,357 cells / `5.743 km2` wet inside the evaluation domain, substantially more than any reference. The result is therefore classified as a retained negative baseline, not a validated flood reconstruction; any physics revision must be separately versioned and reported beside it;
- the deterministic three-panel diagnostic classifies only the already frozen prediction/reference masks into true positive, false positive, false negative, known-dry agreement and outside-evaluation cells. Orange false positives dominate branching flow paths, blue false negatives are more localized, and dark grey remains unavailable rather than dry; SVG SHA-256 is `384506982b9edb2c44f7c69b7e134726f3c6c8a1edc9102327f6997edb161b3e`;
- diagnostic materialization performs zero evaluation runs and network requests, recomputes no metric and changes no model or threshold. It is explicitly barred from model input and from representing a new evaluation;
- the failure inventory uses only the predeclared 0.05 m, 0.10 m and 0.30 m prediction masks plus the pre-event dominant CLC class on the frozen 20 m grid. Across all three references, 78.4-79.3% of false positives remain at or above 0.10 m and 34.0-36.7% remain at or above 0.30 m, so the retained failure is not confined to a shallow threshold fringe;
- the Environment Agency comparison places the largest false-positive counts in pastures, arable land, salt marshes and discontinuous urban fabric. These counts are descriptive and area-dependent, establish no CLC causality, rank no sensitivity scenario and authorize no model change;
- four post-evaluation hypotheses are frozen as unconfirmed and evidence-blocked: channel conveyance representation, boundary/initial state, defence/control state and source-term placement. Each names required event-valid evidence, a forbidden shortcut and a deterministic fixture that must pass before Carlisle can be touched;
- opened Carlisle references are restricted to diagnosis and transparent non-blind comparison. They cannot calibrate, select thresholds or scenarios, accept a revision or support another blind-validation claim; a validation claim requires a new holdout event or area;
- a publication-safe `GET /api/benchmarks/cumbria-2015` snapshot and institutional Case 03 page expose the frozen forcing, prediction, separate metrics, diagnostic image and blocked revision hypotheses without source arrays, external paths, evaluation geometry or execution authority;
...
- use the retained map and inventory to reject a threshold-only explanation, while treating channel conveyance, boundary conditions and defence state as unconfirmed physical hypotheses rather than retrospective corrections;
- any revised experiment must declare its physical hypothesis and acceptance test before execution and preserve the original result beside it.
...
- the 144 half-hour IMERG and 288 quarter-hour Sheepmount samples reproduce from their pinned forcing receipt, retain source resolution and observed values, and cannot authorize solver execution or evaluation access.
- the local-inertial fixture suite reproduces its pinned result and source identities without touching event or evaluation artifacts;
- the event-input binding reproduces its corrected grid identity, native forcing identity, complete valid-cell rainfall coverage, scenario parameter identities, array orientation, units and exact source timing; missing or invalid evidence must fail before Storm Desmond execution.
- the event runner reproduces its pinned source identities and contract, loads at most one forcing interval at a time, streams rather than retains all depth snapshots, writes a checkpoint only after verifying a complete scenario, reuses only a contiguous checkpoint prefix matching every frozen identity, and cannot execute without a clean-revision authorization matching every dependency;
- partial scenario artifacts cannot become a prediction, invalid cells retain explicit NoData rather than dry, and observed evaluation geometry remains inaccessible until the complete prediction receipt exists.
- the prediction-freeze verifier reproduces the final receipt, all nine checkpoint identities, all 72 compressed and decoded artifact identities, primary 0.05 m wet mask, evaluation-domain mask and historical source Git/manifest identity before reference access;
- the evaluation receipt is single-use, content-addressed and checkable without recomputing metrics; all three references remain separate, the negative result cannot be overwritten, and the frozen baseline cannot be relabelled as validated.
- the diagnostic receipt and publication image remain content-addressed derivatives of the frozen masks; drift, metric recomputation, retuning, threshold changes or any attempted evaluation rerun fail closed.
- the failure inventory reproduces only from the frozen 0.05 m, 0.10 m and 0.30 m masks plus the pinned 20 m dominant-CLC artifact; it may not add thresholds, compute replacement evaluation metrics, rank scenarios or authorize a model change.

Checkpoints: `test: qualify Cumbria public-data access`; `test: freeze Cumbria pre-event terrain catalogue`; `test: qualify Cumbria hydraulic context`; `test: freeze Cumbria hydraulic boundary protocol`; `test: qualify Cumbria hydraulic domain lineage`; `test: freeze Cumbria hydraulic model request`; `test: qualify Cumbria DTM archive mapping`; `test: freeze Cumbria DTM materialization protocol`; `test: freeze Cumbria spatial grid boundary`; `feat: compose native spatial evidence index`; `test: freeze Cumbria blind evaluation boundary`; `feat: add Cumbria model delivery intake`; `chore: record Cumbria model request dispatch`; `test: freeze Cumbria public baseline domain`; `feat: materialize Cumbria public terrain`; `feat: materialize Cumbria land cover and precipitation`; `test: freeze Cumbria replacement solver contract`; `feat: materialize Cumbria solver grids`; `feat: materialize Cumbria time-varying forcing`; `feat: verify Cumbria local-inertial kernel`; `feat: bind Cumbria event inputs`; `feat: freeze Cumbria event runner contract`; `refactor: add restart-safe Cumbria scenario checkpoints`; `feat: freeze Cumbria public prediction`; `feat: materialize Cumbria evaluation references`; `feat: normalize Cumbria evaluation references`; `test: retain Cumbria blind evaluation result`; `feat: visualize Cumbria blind evaluation errors`; `test: inventory Cumbria retained failure`; `test: freeze post-evaluation physics revision gate`; `feat: expose Cumbria retained benchmark`
