...
State: the public Sheepmount–Old Sandsfield Storm Desmond run completed all nine frozen scenarios from bounded real evidence; prediction receipt SHA-256 `f2a3a7489699a70a6d5c770633bdc9f789184ca26cb8190c85a1d28d40495dc6` was merged before reference access, the exact Environment Agency and Copernicus sources are now content-addressed separately, and blind evaluation is gated by reference normalization on the frozen grid

Progress:

- manifest v0.27.0 retains the v0.25.0 execution source identity, the v0.26.0 prediction freeze and the later evaluation-reference receipt without rewriting either historical boundary; the primary 20 m wet mask remains maximum depth `>= 0.05 m` over the same 73,502-cell evaluation-domain mask and half-open 72-hour Storm Desmond window;
...
- normalize and rasterize the exact Environment Agency outline, EMSR147 initial delineation and EMSR147 monitoring delineation independently on the frozen 20 m EPSG:27700 grid, preserving explicit missing coverage and content-addressing every transformation;
- only after all three masks pass their geometry and grid checks, evaluate the frozen public-only prediction independently against each reference, reporting negative results and sensitivity without retuning.
