...
State: the public Sheepmount–Old Sandsfield Storm Desmond run completed all nine frozen scenarios from bounded real evidence; prediction receipt SHA-256 `f2a3a7489699a70a6d5c770633bdc9f789184ca26cb8190c85a1d28d40495dc6` was merged before reference access, the exact Environment Agency and Copernicus sources and their independent frozen-grid masks are content-addressed, and the reviewed blind-evaluation executor is byte-pinned pending the single predeclared run without retuning

Progress:

- manifest v0.29.0 retains the v0.25.0 execution source identity, the v0.26.0 prediction freeze, the raw evaluation-reference receipt and the later normalization receipt without rewriting any historical boundary; it additionally binds the reviewed evaluator commit `efe7a05` and Git-blob SHA-256 `7cc63485c59ca3f4716f056ae3f5420f180e55fad62af6dfb006631e11e06373` before any metric run, while the primary 20 m wet mask remains maximum depth `>= 0.05 m` over the same 73,502-cell evaluation-domain mask and half-open 72-hour Storm Desmond window;
...
- evaluate the frozen public-only prediction independently against the normalized Environment Agency, EMSR147 initial and EMSR147 monitoring masks with all six predeclared metrics, reporting negative results and sensitivity without retuning or combining references.
