...
- Cumbria replay qualification gate: metadata, four upstream hydrographs, bounded real DTM/CLC/IMERG evidence, a reproducible real-source H3 inspection cell, a canonically hashed public-only 2D replacement-solver contract, all three static solver grids, the complete 15-minute/30-minute event forcing, an isolated fixture-verified local-inertial kernel, event-input binding, restart-safe scenario checkpointing, blind evaluation isolation and a fail-closed Products 5/6/7 delivery intake are frozen; no model package or evaluation geometry has been received or opened, and a new clean-revision authorization remains required before the checkpointed execution
...
State: public Sheepmount–Old Sandsfield domain, bounded real DTM/CLC/IMERG evidence, corrected 10 m, 20 m and 40 m static solver grids, complete native 15-minute/30-minute forcing, the fixture-verified local-inertial kernel, fail-closed event-input binding and checkpointed event-runner contract are reproduced; a new clean-revision authorization, complete Storm Desmond execution, prediction freeze and blind evaluation remain gated

Progress:

- manifest v0.25.0 freezes a bounded Carlisle source-discovery envelope, a separate local hydraulic-protocol envelope, the public Sheepmount–Old Sandsfield baseline domain, verified DTM/CLC/IMERG materialization and single-cell composition results, the replacement-solver contract, corrected static solver grids, complete time-varying forcing, isolated numerical-kernel verification, event-input binding, restart-safe event-runner contract, and the half-open 72-hour Storm Desmond window from 4 December 2015 00:00 UTC to 7 December 2015 00:00 UTC;
...
- after merge, create the v0.2 content-addressed execution authorization from the clean Git commit and tree without changing the frozen model or output semantics;
- execute every frozen Storm Desmond scenario, retain failure diagnostics, and create no prediction receipt unless all nine pass the exact output schedule, timestep bounds, non-negative finite-depth and per-step/final mass-balance gates;
