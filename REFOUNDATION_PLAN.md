...
- `npm run verify:cumbria-replacement-solver-protocol` reproduces the protocol identity and mesh accounting with zero network requests, zero writes, zero solver runs and zero evaluation-reference loads. Execution remains blocked until the domain grids, 15-minute Sheepmount series and half-hourly IMERG forcing are content-addressed and the numerical kernel passes mass-balance and stability fixtures;
