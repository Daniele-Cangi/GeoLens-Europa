...
State: public Sheepmount–Old Sandsfield domain, bounded real DTM/CLC/IMERG evidence, corrected 10 m, 20 m and 40 m static solver grids, complete native 15-minute/30-minute forcing, the fixture-verified local-inertial kernel and a fail-closed event-input binding are reproduced; the runner/prediction contract, Storm Desmond execution and blind evaluation remain gated

Progress:

- manifest v0.23.0 freezes a bounded Carlisle source-discovery envelope, a separate local hydraulic-protocol envelope, the public Sheepmount–Old Sandsfield baseline domain, verified DTM/CLC/IMERG materialization and single-cell composition results, the replacement-solver contract, corrected static solver grids, complete time-varying forcing, isolated numerical-kernel verification, the event-input binding, and the half-open 72-hour Storm Desmond window from 4 December 2015 00:00 UTC to 7 December 2015 00:00 UTC;
...
- `npm run verify:cumbria-local-inertial-kernel` reproduces fixture result SHA-256 `cd31a9e9cbfb41e44d9619e4a1d9bdd1fc34fa81ce877bbf596772e5c80704c1` for lake-at-rest equilibrium, conservative rainfall, non-negative free outflow, exact source/output alignment and below-minimum timestep failure. It reads zero event inputs and zero evaluation geometries, makes zero network requests or external writes and does not authorize event execution;
...
- event-input receipt SHA-256 `d0ebae08ebef1a1ca08b83977b9110eedbcf70d9290964c05ae20f8ca022500a` binds all nine scenarios through 19 unique content-addressed mapping artifacts: 399,523 valid cells have complete exact-footprint IMERG coverage, source timing is fixed at 288 left-closed 900-second intervals, and Sheepmount discharge is distributed only by intersected valid footprint area;
...
- freeze the exact event-runner inputs/outputs, stability and mass-balance verification procedure, code revision and prediction artifact contract before allowing the first event run;
