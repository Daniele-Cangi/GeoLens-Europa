...
- declared IMERG, CLC, GLO-30, XDBTR and event-2 coverage all contain the bounded pilot; manifest v1.2.0 originally froze the common EPSG:4326 bounds, which current manifest v1.13.0 retains together with a globally aligned EPSG:32632 30 m grid of 335 by 420 cells, while H3 r11 remains a separate representation choice;
...
- manifest v1.13.0 and the external-data verifier now pin 54 benchmark artifacts totaling 716,260,635 bytes, including the WCS description, bounded PST audit, official breach-repair HTML, special-plan monograph, PAI relation and three inspected DWG archives.
...
- acquire an event-valid Montone/Rabbi discharge relation or hydrograph, a defensible downstream boundary, source-backed breach timing/geometry, and a complementary terrain/channel source before the frozen conditioned replay can become eligible; the public PAI drawing archives do not satisfy this requirement;
