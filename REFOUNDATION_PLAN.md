...
State: public Sheepmount–Old Sandsfield domain, bounded real DTM/CLC/IMERG evidence, the frozen 10 m, 20 m and 40 m static solver grids, the complete native 15-minute/30-minute event forcing and an isolated fixture-verified local-inertial kernel are reproduced; a single real native-footprint H3 inspection cell and the experimental replacement-solver contract remain content-addressed, while event-input binding, Storm Desmond execution and blind evaluation remain gated

Progress:

- manifest v0.22.0 freezes a bounded Carlisle source-discovery envelope, a separate local hydraulic-protocol envelope, the public Sheepmount–Old Sandsfield baseline domain, verified DTM/CLC/IMERG materialization and single-cell composition results, the replacement-solver contract, its static solver grids, its complete time-varying forcing, the isolated numerical-kernel verification, and the half-open 72-hour Storm Desmond window from 4 December 2015 00:00 UTC to 7 December 2015 00:00 UTC;
...
- bind the materialized solver grids and time-varying forcing to the verified local-inertial kernel with explicit units, masks, source timing, content identities and a fail-closed missing-data policy before allowing an event run;
