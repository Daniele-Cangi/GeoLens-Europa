...
- Historical benchmark spatial-inspection gate: established through a deterministic nominal 300 m display projection from byte-verified 30 m AOI, GLO-30, CLC, DBTR and terrain-routing artifacts; the map manifest exposes four attribution-bearing renderable layers while V7 geometry, event-runoff spatial values and ARPAE station geometry remain explicitly withheld under restricted or unresolved redistribution state
...
- declared IMERG, CLC, GLO-30, XDBTR and event-2 coverage all contain the bounded pilot; manifest v1.2.0 originally froze the common EPSG:4326 bounds, which current manifest v1.15.0 retains together with a globally aligned EPSG:32632 30 m grid of 335 by 420 cells, while H3 r11 remains a separate representation choice;
...
- a deterministic publication-safe spatial projection aggregates verified source cells in 10 by 10 blocks without changing native-resolution claims; its API and Case 02 canvas inspector expose terrain-only D8 contributing area, mean GLO-30 elevation, dominant CLC group and DBTR known-water presence, while restricted V7 geometry and review-pending event/station spatial values remain registered with null data and explicit reasons.
