...
- Cumbria replay qualification gate: metadata, four upstream hydrographs, bounded real DTM/CLC/IMERG evidence, a reproducible real-source H3 inspection cell, a canonically hashed public-only 2D replacement-solver contract, all three static solver grids, the complete 15-minute/30-minute event forcing, blind evaluation isolation and a fail-closed Products 5/6/7 delivery intake are frozen; no model package or evaluation geometry has been received or opened, while kernel verification and event execution remain blocked
...
State: public Sheepmount–Old Sandsfield domain, bounded real DTM/CLC/IMERG evidence, the frozen 10 m, 20 m and 40 m static solver grids, and the complete native 15-minute/30-minute event forcing are materialized; a single real native-footprint H3 inspection cell and the experimental replacement-solver contract are reproduced, while kernel verification, event execution and blind evaluation remain gated

Progress:

- manifest v0.21.0 freezes a bounded Carlisle source-discovery envelope, a separate local hydraulic-protocol envelope, the public Sheepmount–Old Sandsfield baseline domain, verified DTM/CLC/IMERG materialization and single-cell composition results, the replacement-solver contract, its static solver grids, its complete time-varying forcing, and the half-open 72-hour Storm Desmond window from 4 December 2015 00:00 UTC to 7 December 2015 00:00 UTC;
...
- `npm run verify:cumbria-replacement-solver-protocol` reproduces the protocol identity and mesh accounting with zero network requests, zero writes, zero solver runs and zero evaluation-reference loads. The domain grids and forcing are now content-addressed; execution remains blocked until the numerical kernel passes mass-balance and stability fixtures;
...
Checkpoints: `test: qualify Cumbria public-data access`; `test: freeze Cumbria pre-event terrain catalogue`; `test: qualify Cumbria hydraulic context`; `test: freeze Cumbria hydraulic boundary protocol`; `test: qualify Cumbria hydraulic domain lineage`; `test: freeze Cumbria hydraulic model request`; `test: qualify Cumbria DTM archive mapping`; `test: freeze Cumbria DTM materialization protocol`; `test: freeze Cumbria spatial grid boundary`; `feat: compose native spatial evidence index`; `test: freeze Cumbria blind evaluation boundary`; `feat: add Cumbria model delivery intake`; `chore: record Cumbria model request dispatch`; `test: freeze Cumbria public baseline domain`; `feat: materialize Cumbria public terrain`; `feat: materialize Cumbria land cover and precipitation`; `test: freeze Cumbria replacement solver contract`; `feat: materialize Cumbria solver grids`; `feat: materialize Cumbria time-varying forcing`
